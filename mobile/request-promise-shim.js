...
console.log('[request-promise-shim] Loading request-promise shim module');
...
function jar() {
  return {
    setCookie: function(cookie, url) {
      console.log('[request-promise-shim] jar.setCookie called (noop):', cookie, url);
    },
    getCookies: function(url) {
      console.log('[request-promise-shim] jar.getCookies called (noop):', url);
      return [];
    }
  };
}
...
function requestPromise(options) {
  console.log('[request-promise-shim] request called with options:', options);
...
requestPromise.defaults = function(defaultOptions) {
  console.log('[request-promise-shim] defaults called with:', defaultOptions);
...
['get', 'post', 'put', 'delete', 'patch', 'head'].forEach(function(method) {
  requestPromise[method] = function(url, options) {
    return requestPromise(Object.assign({}, options, { method: method.toUpperCase(), url: url }));
  };
});

module.exports = requestPromise;

console.log('[request-promise-shim] Request-promise shim module loaded successfully');

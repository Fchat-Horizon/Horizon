 ...
 *
 * * Pure fs/Buffer logic with no dependency on renderer state, so it can be
 * * used both by the main-process filesystem host and by the renderer-side
 * * Slimcat importer. Callers supply the log directory and resolve character
 * * objects themselves.
 */

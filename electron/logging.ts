/**
 * @module logging
 * Wires the shared logger to electron-log on the host: the shared logger owns
 * the console, electron-log is only the file sink (its console transport is
 * off to avoid double-printing). Renderer records reach main over IPC.
 */

import type { LogAdapter, LevelOption } from '../logger';
import {
  combineAdapters,
  detectLogAdapter,

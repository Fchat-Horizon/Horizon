...
export function initFilesystemHost(opts: {
  ...
  /* Returns whether any index file was corrupt so the renderer can alert. */
  ...
  /* ^ Fire-and-forget append is safe: IPC messages arrive in send order and
     this handler is synchronous, so a later read can't overtake us. */
  ipc.on(
    'logs-log-message',
    (
      _e,
  ...
  /* Dev-only theme hot reload: renderers re-read on change. */
  ...
  ipc.on('eicons-write', (_e, content: string) => {
    try {
      ...
      // Not a showstopper; cache write failure is non-fatal.

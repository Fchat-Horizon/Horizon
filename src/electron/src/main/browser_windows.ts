...
import { getSafeLanguages, updateSupportedLanguages } from '@common/language';
...
export function createMainWindow(
  ...
  switch (process.platform) {
    // Windows and macOS use different property names for the window color.
    case 'win32':
    ...
    // Linux case; placed as the default so TypeScript sees the variable as
    // always assigned.
    default:
  ...
  // ^ On macOS the app menu is not bound to a window, so it stays active with
  // no focused window; toggle window-specific items by hand.
  if (process.platform === 'darwin') {
    window.on('show', () => {
...
export function createAboutWindow(
  ...
  const aboutWindowProperties: electron.BrowserWindowConstructorOptions = {
    ...
    webPreferences: {
      ...
      sandbox: true,
      spellcheck: true
      // Stay on the default session: persist:fchat carries the chat's per-host
      // zoom, which this fixed-size dialog would inherit and render at the wrong
      // scale. The default session has its own zoom map and serves app:// too.
    }
  };

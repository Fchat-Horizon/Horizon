/**
 * Build script for the Horizon Electron app. Targets different operating
 * systems and architectures, with or without Docker, via electron-builder.
 *
 *       Usage:
...
async function runNativeBuild(opts, targetKey) {
  ...
  // ^ Our OS keys differ from electron-builder's platform property names.
  const platformMap = {
    linux: 'linux',
    macos: 'mac',
    windows: 'win'
  ...
  // ^ Reuse package.json build config as base, overriding only targets.
  const config = {
    ...baseConfig,

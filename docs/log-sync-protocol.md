...
Horizon extends a conversation in place rather than rewriting it whenever it can, appending to the log and continuing the day index. It does this only when a bounded scan of the stored log proved it undamaged and in time order, the index matched a rebuild from that log, the file has not changed since, and the incoming batch is ascending and starts at or after the last stored message. Any of those failing falls back to the rewrite below, so the result is identical either way. Because the scan establishes all of this without parsing the conversation into memory, the first batch to touch a conversation can extend it too, not only later ones. An extension grows the log and flushes it before the index, the opposite order to a rewrite: a crash leaving the log grown and the index short only costs a day marker until Fix Logs runs, whereas an index entry pointing past the end of a short log would be read as garbage messages.

On Horizon, merged conversations are otherwise rewritten in the binary log format of `electron/filesystem.ts` and the `.idx` day index is rebuilt in the same pass (`electron/services/sync/log-merge.ts`). That rewrite works one day at a time rather than on the whole conversation: since the `.idx` already partitions a conversation by day, and a day stays small even for a busy channel, the merge reads only the day it is currently writing. Deduplicating per day is equivalent to deduplicating against the whole file, because the dedupe tuple begins with `time` and equal times fall in the same day. A day with nothing new whose records are already one run in time order is copied byte for byte; every other day is read, merged, sorted and re-serialized on its own. The day table is rebuilt by scanning the log rather than trusted from the stored index, because a stale `.idx` is a real failure mode. Peak memory is therefore set by the largest single day plus the incoming batch, not by the size of the conversation. Both replacements are prepared before changing the original files. If installation fails, Horizon restores the original pair. The old index is removed before replacing the log so stale offsets cannot accompany a new log. This is not a filesystem transaction across two files: interruption by a process or machine crash can leave a missing index requiring Fix Logs; recovery copies remain in a hidden `.sync-*` directory if installation or rollback is interrupted.
...
- A batch targets 16 MiB of uncompressed JSON, cut after the record that crosses that, and at most 350000 records. The budget is counted on serialized JSON rather than binary log bytes, because JSON escaping is what the receiver has to allocate. The record allowance is a backstop on allocation rather than the limit a batch is meant to stop on; it sits above the point where the smallest possible records would exhaust the byte budget, so a batch of short messages still fills its 16 MiB. Each sender picks its own batch size; nothing requires the two to agree.
